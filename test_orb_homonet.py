...
OUT_DIR    = "./warp_results_maxITERs3000"
...
def orb_estimate_homography(Ip, Ip_p,
    ...
    # 2) ORB 특징점 검출 및 descriptor 생성
    orb = cv.ORB_create(nfeatures=nfeatures, edgeThreshold=15, fastThreshold=7)
    kp1, des1 = orb.detectAndCompute(Ip, None)  #
    kp2, des2 = orb.detectAndCompute(Ip_p, None)
    if des1 is None or des2 is None or len(kp1) < 4 or len(kp2) < 4:
        return None, "No descriptors"

    # 3) KNN + Lowe ratio test : 디스크립터끼리 비교하여 가까운 2개 가져와 대응점으로 본다, 대응점 4개 미만이면 match 불가 -> 호모그래피 못찾음
    ...
    pts1 = np.float32([kp1[m.queryIdx].pt for m in good]).reshape(-1, 1, 2)     #매칭된 keypoint를 호모그래피 계산에 맞는 형태로 뽑기
...
def draw_boxes_on_image(img_gray, gt_xy, pred_xy, title, ace_px,
    ...
    cv.polylines(vis, [gt_xy.astype(np.int32).reshape(-1,1,2)],  True, color_gt,   1, cv.LINE_AA)   #blue : gt
...
def make_paper_style_viz_full(full_img, patch_tensor, p_xy, v_pred8, v_gt8,
    ...
    # ORB로 H_orb 추정 → 원본 좌표로 변환
    H_orb, err = orb_estimate_homography(Ip, Ip_p)
    if H_orb is not None:
        dst_orb_rel  = cv.perspectiveTransform(base_rel.reshape(1,4,2), H_orb)[0] #원본패치를 H_orb적용 -> orb추정 사각형 get
        dst_orb_full = dst_orb_rel + p  # orb추정 사각형을 원본좌표로 평행이동
        ace_orb = float(np.linalg.norm(dst_orb_full - gt_full, axis=1).mean())  #MACE 계산
        left  = draw_boxes_on_image(full_img, gt_full, dst_orb_full, "Traditional (ORB)", ace_orb)
    else:
        dst_orb_full = base_rel + p
        ace_orb = float(np.linalg.norm(dst_orb_full - gt_full, axis=1).mean())  #np.linalg.norm : 벡터 길이 계산
        left  = draw_boxes_on_image(full_img, gt_full, dst_orb_full, f"Traditional (ORB) — {err}", ace_orb)

    # Homographynet 으로 H 추정
    ace_net = float(np.linalg.norm(pred_full - gt_full, axis=1).mean())
    right   = draw_boxes_on_image(full_img, gt_full, pred_full, "Deep (HomographyNet)", ace_net)

    #rectify용 사각형
    base_full = base_rel + p  # (4,2)

    # (1) ORB 직교화: ORB 추정 사각형(dst_orb_full)을 base_full로 펴기
    if H_orb is not None:
    ...
    # (2) HNet 직교화: HNet 예측 사각형(pred_full)을 base_full로 펴기
...
def prediction(filename):
    ...
    for (img, target) in tqdm(test_loader):
        ...
        for i in range(out_np.shape[0]): #out_np.shape[0] 배치크기만큼을 반복하며
            ...
            _ = make_paper_style_viz_full(full_img_bgr, img[i], p_xy, v_pred, v_gt,
